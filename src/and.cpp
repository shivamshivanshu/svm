...
namespace svm
{
    Trap SingleCore::AND_rr(Regs first, Regs second) noexcept
    {
    ...
    Trap SingleCore::AND_mr(MemoryAddress addr, Regs second) noexcept
    {
    ...
    Trap SingleCore::AND_rm(Regs first, MemoryAddress addr) noexcept
    {
    ...
    Trap SingleCore::AND_ri(Regs first, Immediate value) noexcept
    {
        auto regValue = readRegister(first);
        regValue &= value;
        writeRegister(first, regValue);
    ...
    Trap SingleCore::AND_mi(MemoryAddress addr, Immediate value) noexcept
    {
        auto [trap, lvalue] = mem->read(addr);
        if (trap == Trap::OK)
        {
