...
namespace svm
{
    Trap SingleCore::OR_rr(Regs first, Regs second)
    {
    ...
    Trap SingleCore::OR_mr(MemoryAddress addr, Regs second)
    {
    ...
    Trap SingleCore::OR_rm(Regs first, MemoryAddress addr)
    {
    ...
    Trap SingleCore::OR_ri(Regs first, Immediate value)
    {
        auto regValue = readRegister(first);
        regValue |= value;
        writeRegister(first, regValue);
    ...
    Trap SingleCore::OR_mi(MemoryAddress addr, Immediate value)
    {
        auto [trap, lvalue] = mem->read(addr);
        if (trap == Trap::OK)
        {
